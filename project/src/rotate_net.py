#
# code from https://github.com/d4nst/RotNet
#
...
class RotateNet:
    ...
    def load_model(self, path='models/model_rotnet.hdf5'):
        self.model = load_model(path, custom_objects={'angle_error': angle_error})
    ...
    def train(self):
        ...
        x = Conv2D(self.nb_filters, self.kernel_size, activation='relu')(input)
        x = Conv2D(self.nb_filters, self.kernel_size, activation='relu')(x)
        ...
        self.checkpointer = ModelCheckpoint(
            filepath=os.path.join(output_folder, 'model_rotnet.hdf5'),
            save_best_only=True
        )
        ...
        self.model.fit_generator(
            ...
            verbose=1,
            callbacks=[self.checkpointer, self.early_stopping, self.tensorboard]
        )

    def correct_rotation(self, image):
        predictions = self.model.predict(image.reshape(1, 28, 28, 1))
        predicted_angle = np.argmax(predictions)
        image = image.reshape(28, 28)
        image_center = tuple(np.array(image.shape[1::-1]) / 2)
        rot_mat = cv2.getRotationMatrix2D(image_center, -predicted_angle, 1.0)
